...
class Mini3DViewer:
    ...
    def define_gui(self):
        ...
        with dpg.window(label="viewer", tag="_canvas_window", width=self.W, height=self.H, no_title_bar=True,
        ...
        def callback_mouse_move(sender, app_data):
            ...
            if self.drag_begin_x is not None or self.drag_begin_y is not None:
                ...
                if self.drag_button is dpg.mvMouseButton_Left:
                    ...
                    if self.W * k < self.drag_begin_x < self.W * (1 - k) and self.H * k < self.drag_begin_y < self.H * (
                            1 - k):
