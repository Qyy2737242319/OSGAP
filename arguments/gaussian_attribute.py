...
class GaussianAttributeConfig(Config):
    use_rodriguez_rotation: bool = True  # If set, predict rodriguez rotation in [0, 2pi] instead of quaternions
    sh_degree: int = 2
...
class GaussianAttribute(StringEnum):
    ...
    def get_n_channels(self, config: GaussianAttributeConfig) -> int:
        if self == self.POSITION:
            ...
            n_channels_per_color = (config.sh_degree + 1) ** 2 + 4# for in total uv
