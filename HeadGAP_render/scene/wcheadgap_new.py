...
from arguments.infer_config import get_parser
from arguments.swin_config import get_config
from scene.goae_encoder import GOAEncoder as GOAEncoder
...
class WCHeadGAPModel(nn.Module):
    def __init__(self, config, avatar_id_list, part_id_list, cuda_index=0, n_shape=300, n_expr=100):
        ...
        self.Encoder = GOAEncoder(swin_config, mlp_layer=opts.mlp_layer, stage_list=[0, 0, 0]).to("cuda:0")
    ...
    def fintune_w_setup(self):
        ...
        self.optimizer = torch.optim.Adam([
            {'params': self.Encoder.parameters(), 'lr': 0.0005},  # 为 latent_z 设置学习率 0.001
        ])
    ...
    def fintune_network(self, cameras, background: torch.tensor, usecnn=False):
        ...
        latent = F1.resize(input_image, [3, 512])
        latent = torch.mean(latent, dim=1).flatten()
        w = latent[:self.latent_f.shape[0] * 11].reshape(self.latent_f.shape[0], -1, 1)

        softmax_weights = F.softmax(w, dim=0)  # 计算 softmax 权重，形状 [k, p, 1]
