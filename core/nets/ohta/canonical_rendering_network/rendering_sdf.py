...
from core.nets.ohta.embedders.log_fourier import get_embedder
...
class RenderNetwork(nn.Module):
    ...
    def forward(self, points, normals, view_dirs, feature_vectors, sigmoid=True):
        ...
        if self.squeeze_out and sigmoid:
            x = torch.sigmoid(x)
        return x