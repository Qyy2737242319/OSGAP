...
class Partitioner(torch.nn.Module):
    ...
    def __init__(self, parametric_body: smplx.SMPL, smpl_cfg: dict):
        ...
        # Multi-resolution global sample        
        self.sample_point_num_list = cfg.shadow_network.dict_list + cfg.color_network.dict_list 
        for sample_point_num in self.sample_point_num_list: 
            sample_points, (sample_face_idxs, w0, w1, w2) = self.global_sample_mesh_points(parametric_body.v_template, parametric_body.faces_tensor, sample_point_num)
            self.register_buffer(f'sample_face_tensor_{sample_point_num}', parametric_body.faces_tensor[sample_face_idxs[0]].long())
            self.register_buffer(f'w0_{sample_point_num}', w0)
            self.register_buffer(f'w1_{sample_point_num}', w1)
            self.register_buffer(f'w2_{sample_point_num}', w2)
            joints = torch.mm( parametric_body.J_regressor, parametric_body.v_template)
            self.register_buffer(f'sample_points_{sample_point_num}', sample_points[0] - joints[4:5])
    ...
    def get_model_type(parametric_body):
        if 'MANO' in str(type(parametric_body)):
            model_type = 'mano'
        elif 'FLAME' in str(type(parametric_body)):
            model_type = 'flame'
        elif 'SMPLX' in str(type(parametric_body)):
            model_type = 'smplx'
        elif 'SMPLH' in str(type(parametric_body)):
            model_type = 'smplh'
        elif 'SMPL' in str(type(parametric_body)):
            model_type = 'smpl'
        else:
...
class COAPBodyModel(torch.nn.Module):
    ...
    def __init__(self, parametric_body: smplx.SMPL, smpl_cfg: dict) -> None:
        ...
        self.decoder_shape = ImplicitNet(
            d_in=smpl_cfg['local_en_dim']+(0,2)[smpl_cfg['inbox_feat']]+10,
            d_out=(1, 2)[smpl_cfg['fusion_pool'] ],
            dims=[smpl_cfg['decoder_layer'][1],]*smpl_cfg['decoder_layer'][0],
            skip_in=[smpl_cfg['decoder_layer'][0]//2],
        )
    ...
    def encode_body(self, smpl_output):
        ...
        # multi-resolution global points
        multi_global_points = []
        for sample_point_num in self.partitioner.sample_point_num_list:
            face_verts = vertices[:, self.partitioner.__getattr__(f'sample_face_tensor_{sample_point_num}')]
            v0, v1, v2 = face_verts[:, :, 0], face_verts[:, :, 1], face_verts[:, :, 2]
            global_points = \
                self.partitioner.__getattr__(f'w0_{sample_point_num}')[:, :, None] * v0 + \
                self.partitioner.__getattr__(f'w1_{sample_point_num}')[:, :, None] * v1 + \
                self.partitioner.__getattr__(f'w2_{sample_point_num}')[:, :, None] * v2
            multi_global_points.append(global_points.clone())
        ...
        self.impl_code = dict(
            ...
            multi_global_points = multi_global_points,
        )
    ...
    def query(self, points, smpl_output=None, ret_intermediate=False):
        ...
        # Multi-resolution 
        multi_global_points = self.impl_code['multi_global_points']
        gdists_list = []
        gindices_list = []
        for global_points in multi_global_points:
            ...
            gdists_list.append(gdists.clone())
            gindices_list.append(gindices.clone())
        ...
        if valid.sum() == 0:
            ...
            part_pred.update({'part_idx': torch.zeros([B*T]).to(points.device), 'gdists_list': gdists_list, 'gindices_list': gindices_list})
        ...
        # get part index
        index = part_occupancy.max(dim=1).indices[0].long().detach()

        part_pred = {'part_occupancy': part_occupancy, 'all_out': ~inside_bbox.any(1), 'gdists': gdists, 'gindices': gindices, 'part_idx': index,  'gdists_list': gdists_list, 'gindices_list': gindices_list}
        # t7 = time.time()
        if self.soft_act is not None:
