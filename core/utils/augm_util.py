...
def augmentation(img, bbox, data_split, exclude_flip=False, input_img_shape=(256, 256), mask=None, base_scale=1.1, scale_factor=0.25, rot_factor=60, shift_wh=None, gaussian_std=1, color_aug=False, bordervalue=(0,0,0)):
    ...
    img, trans, inv_trans, mask, shift_xy = generate_patch_image(img, bbox, scale, rot, shift, do_flip, input_img_shape, shift_wh=shift_wh, mask=mask, bordervalue=bordervalue)
    if color_aug:
...
def augmentation_kps(img, joint_img, joint_cam, joint_valid, hand_type, joint_type, trans, do_flip, input_shape=(256, 256), princpt=None):
    joint_img = joint_img.copy()
    ...
    for i in range(joint_num):
        joint_img[i,:2] = trans_point2d(joint_img[i,:2], trans)
        joint_valid[i] = joint_valid[i] * (joint_img[i,0] >= 0) * (joint_img[i,0] < input_shape[1]) * (joint_img[i,1] >= 0) * (joint_img[i,1] < input_shape[0])
    if princpt is not None:
        if do_flip:
            princpt[0] = original_img_shape[1] - princpt[0] - 1
        princpt = trans_point2d(princpt, trans)
