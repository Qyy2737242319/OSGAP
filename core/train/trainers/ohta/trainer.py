...
from core.utils.metric_util import ssim_metric

from configs import cfg
from pytorch3d.structures import Meshes, Pointclouds
from pytorch3d.loss import mesh_laplacian_smoothing, mesh_normal_consistency
from pytorch3d.renderer import PerspectiveCameras, PointsRasterizationSettings, PointsRasterizer, AlphaCompositor, PointsRenderer
...
class Trainer(object):
    def __init__(self, network, optimizer, board=None):
        ...
        if (cfg.resume or cfg.phase=='val'):
    ...
    def get_loss(self, net_output, patch_masks, bgcolor, targets, 
        ...
        if 'shadow' in net_output:
            shadow = net_output.pop('shadow')[..., None].repeat(1, 3)
            losses['shadow_reg'] = torch.abs(shadow - 1).mean()
    ...
    def progress(self):
        ...
        for step, batch in enumerate(self.prog_dataloader):
            ...
            # # # # Use masked results for evaluation.
            # mask = batch['target_alpha_img'][:, :, None].numpy()
    ...
    def load_ckpt(self, name):
        ...
        new_sd = self.network.state_dict()  # take the "default" state_dict
