...
class WCHeadGAPModel(nn.Module):
    ...
    def fintune_w_setup(self):
        self.w = torch.nn.Parameter(torch.zeros(1, 11, 1).cuda(self.cuda_index))
        #self.optimizer = torch.optim.Adam([self.w], lr=0.00001) #0.001
    ...
    def fintune_network(self, cameras, background: torch.tensor, usecnn=False):
        ...
        for camera in cameras:
            c2w = torch.zeros((4, 4))
            c2w[:3, :3] = torch.tensor(camera.R)
            c2w[:3, 3] = torch.tensor(camera.T)
            c2w[3, 3] = 1
            extrinsics = c2w.flatten()
            intrinsics = cameras[0].projection_matrix[:3, :3].flatten()
            c = torch.cat([extrinsics, intrinsics], dim=0).unsqueeze(0).cuda()
            if cs is not None:
        ...
        rot_delta_uv = rot_delta_uv.reshape(-1,4,9,256,256).reshape(-1,4,256,256)
        local_scaling_uv = local_scaling_uv.reshape(-1,3,9,256,256).reshape(-1,3,256,256)
        gs_opacity_uv = gs_opacity_uv.reshape(-1,1,6,256,256).reshape(-1,1,256,256)
        ...
        w_ = self.mapping(w, cs, truncation_psi=truncation_psi)

        planes = None
        for c_batch, w_batch in zip(batchify_sliced(cs, batch_size=2),
                                    batchify_sliced(w_, batch_size=2)):
            plane = self.generate_planes(w_batch, c_batch, return_masks=True, noise_mode='const',  # 8 22 256 256
                                          neural_rendering_resolution=512)
            if planes is not None:
                planes = torch.cat([planes, plane], dim=0)
            else:
                planes = plane
        ...
        z_xs = torch.linspace(-1, 1, steps=self.config.n_uniform_flame_vertices, device="cuda")
        z_ys = torch.linspace(-1, 1, steps=self.config.n_uniform_flame_vertices, device="cuda")

        z_xs, z_ys = torch.meshgrid(z_xs, z_ys, indexing='ij')
        z_sampled_uv_coords = torch.stack([z_ys, z_xs], dim=-1)
        z_sampled_uv_coords = torch.flatten(z_sampled_uv_coords, start_dim=0, end_dim=1)

        f_xs = torch.linspace(-1, 1, steps=11, device="cuda")
        f_ys = torch.linspace(-1, 1, steps=128, device="cuda")

        f_xs, f_ys = torch.meshgrid(f_xs, f_ys, indexing='ij')
        f_sampled_uv_coords = torch.stack([f_ys, f_xs], dim=-1)
        f_sampled_uv_coords = torch.flatten(f_sampled_uv_coords, start_dim=0, end_dim=1)
        f_valid_uv_coords = f_sampled_uv_coords.unsqueeze(0).unsqueeze(2).contiguous()
        f_valid_uv_coords = f_valid_uv_coords.repeat(f_uv.shape[0], 1, 1, 1)

        latent_f_sampled = torch.nn.functional.grid_sample(f_uv, f_valid_uv_coords,
                                                   align_corners=False,
                                                   mode="bilinear")  # [B*S, C_uv, G, 1]
        latent_f_sampled = latent_f_sampled.reshape(-1, 11, 128)
        ...
        for part_index in range(11):
            ...
            z_valid_uv_coords = z_sampled_uv_coords[sampled_indices]
            z_valid_uv_coords = z_valid_uv_coords.unsqueeze(0).unsqueeze(2).contiguous()


            latent_z = torch.nn.functional.grid_sample(z_uv, z_valid_uv_coords,
                                                       align_corners=False,
                                                       mode="bilinear")  # [B*S, C_uv, G, 1]
            latent_z = latent_z.squeeze().permute(1, 0)

            latent_f = latent_f_sampled[:,part_index,:]
            ...
            e = global_position - cano_vertices_xyz[:, gs_indices, :]
    ...
    def fintune_w(self, cameras: Camera, background: torch.tensor, usecnn=False):
        ...
        for part_index in range(11):
            ...
            # uv_z = torch.nn.functional.interpolate(uv_z, size=(512, 512), mode='bicubic',align_corners=True)
    ...
    def forward(self, cameras: Camera, background: torch.tensor, usecnn=True):
        ...
        for part_index in range(11):
            ...
            z_valid_uv_coords = z_sampled_uv_coords[sampled_indices]
            z_valid_uv_coords = z_valid_uv_coords.unsqueeze(0).unsqueeze(2).contiguous()
            z_valid_uv_coords = z_valid_uv_coords.repeat(z_uv.shape[0], 1, 1, 1)
            ...
            latent_z = torch.nn.functional.grid_sample(z_uv, z_valid_uv_coords,
                                                       align_corners=False,
                                                       mode="bilinear")  # [B*S, C_uv, G, 1]
            latent_z = latent_z.squeeze().permute(0, 2, 1)
            #latent_f_origin = latent_f_B[:, [part_index], :]
            ...
            B, d = latent_f.shape
            B, N, D = latent_z.shape

            latent_z_expanded = latent_z.expand(B, N, D)
            latent_f_expanded = latent_f.unsqueeze(1).expand(B, N, d)
            input = torch.cat([latent_z_expanded, latent_f_expanded], dim=-1)
