...
from roma import quat_product, quat_xyzw_to_wxyz, quat_wxyz_to_xyzw,quat_inverse
...
class GaussianModel:
    ...
    def inverse_scaling(self, global_scaling):
        if self.binding is None:
            # 使用对数函数作为逆操作
            return torch.log(global_scaling)
        else:
            if self.face_scaling is None:
                self.select_mesh_by_timestep(0)

            # 先恢复到局部缩放，再应用对数函数
            local_scaling = global_scaling / self.face_scaling[self.binding]
            return torch.log(local_scaling)

    def inverse_rotation(self, B):
        """
        Compute the inverse transformation to obtain the local rotation (self._rotation)
        from the global rotation quaternion (B).

        Args:
            B (tensor, XYZW convention): Global rotation quaternion (...x4 tensor).
        Returns:
            Local rotation quaternion (self._rotation) in XYZW convention (...x4 tensor).
        """
        if self.binding is None:
            # 无绑定情况下，局部旋转即为 B 自身
            return self.rotation_activation(B)
        else:
            # 有绑定情况下，反推局部旋转 self._rotation
            # 获取绑定对象的全局方向四元数
            face_orien_quat = self.rotation_activation(self.face_orien_quat[self.binding])
            # 转换为 XYZW 格式并计算 face_orien_quat 的逆
            globalresult= quat_wxyz_to_xyzw(B)
            face_orien_quat_inv = quat_inverse(quat_wxyz_to_xyzw(face_orien_quat))
            # 计算局部旋转四元数
            local_rotation = quat_product(face_orien_quat_inv, globalresult)
            # 返回标准化后的局部旋转
            return quat_xyzw_to_wxyz(local_rotation)


    
    def inverse_xyz(self, xyz_global):
        if self.binding is None:
            return self._xyz
        else:
            ...
            # 计算局部坐标
            xyz_local = (xyz_global - self.face_center[self.binding]) / self.face_scaling[self.binding]
            xyz_local = torch.bmm(self.face_orien_mat[self.binding].transpose(1, 2), xyz_local[..., None]).squeeze(-1)
            return xyz_local
