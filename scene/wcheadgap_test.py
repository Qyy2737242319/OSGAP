...
class WCHeadGAPModel(nn.Module):
    def __init__(self, config, avatar_id_list, part_id_list, cuda_index=0, n_shape=300, n_expr=100, uv_gt_path=None, uv_gt_list_path=None):
        ...
        if uv_gt_list_path is not None:
            self.uv_gt_list_path = uv_gt_list_path
            self.uv_gt_num = len(self.uv_gt_list_path)
    ...
    def local2global_xyz(self, local_xyz):
        xyz = torch.matmul(self.face_orien_mat[:, self.binding, :, :],  local_xyz[..., None]).squeeze(-1)
        return xyz * self.face_scaling[:, self.binding, :] + self.face_center[:, self.binding, :]

    def local2global_rotation(self, local_rotation):
        rot = self.rotation_activation(local_rotation, dim=-1)
        face_orien_quat = self.rotation_activation(self.face_orien_quat[:, self.binding, :], dim=-1)
        # rot=local_rotation
    ...
    def local2global_scale(self, local_scale):
        local_scale = self.scaling_activation(local_scale)
        return local_scale * self.face_scaling[:, self.binding, :]  # +0.001
    ...
    def merge_uv_params(self, camera_list):

        uv_gt =[]
        for camera in camera_list:
            uv_gt.append(np.load(os.path.join(self.uv_gt_list_path,f"{camera.avatar_id}.npz")))

        return uv_gt


    def finetune_rgb_setup(self):
    ...
    def fintune_network(self, cameras, background: torch.tensor, usecnn=False):
        ...
        if self.uv_gt_list_path is not None:
            self.uv_gt = self.merge_uv_params(cameras)
        ...
        for part_index in range(11):
            ...
            valid_uv_coords = sampled_uv_coords[self.mask_index].flatten()
    ...
    def finetune(self, cameras, front_camera, background: torch.tensor, usecnn=False):
        gt_rgb = []
        gt_opecity = []
        ...
        valid_uv_coords = sampled_uv_coords[self.mask_index[...,0],self.mask_index[...,1],:]
        valid_uv_coords = valid_uv_coords.unsqueeze(0).unsqueeze(2).contiguous()
        ...
        global_features = attr[..., 11:]
        #h[..., :3] = torch.sigmoid(h[..., :3])

        global_alpha = self.opacity_activation(gs_op)
        global_xyz = self.local2global_xyz(local_xyz)
        global_rotation = self.local2global_rotation(local_rots)
        global_scale = self.local2global_scale(local_scalings)
    ...
    def forward(self, cameras: Camera, background: torch.tensor, usecnn=True):
        ...
        uv_gt = torch.cat(
            [local_position_uv_gt, rot_delta_uv_gt, local_scaling_uv_gt, gs_opacity_uv_gt, gs_features_uv_gt],
