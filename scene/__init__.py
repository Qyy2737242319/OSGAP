...
class CameraDataset(torch.utils.data.Dataset):
    def __init__(self, cameras: List[Camera],load_seg=False):
    ...
    def label_to_one_hot(self,label,num_classes=11):


        # 转换为 one-hot 标签
        one_hot_label = torch.nn.functional.one_hot(label, num_classes=num_classes)

        # 如果需要交换维度使得 (num_classes, h, w)
        one_hot_label = one_hot_label.permute(2, 0, 1)  # 变成 (num_classes, h, w)
    ...
    def __getitem__(self, idx):
        if isinstance(idx, int):
            ...
            if camera.image is None:
                ...
                image = camera.image
            mask_path = camera.image_path.replace("images", "fg_masks")
            ...
            resized_image_mask= PILtoTorch(mask_image, (camera.image_width, camera.image_height))
            ...
            camera.mask=resized_image_mask.clamp(0.0, 1.0)
