...
class FlameHead(nn.Module):
    ...
    def __init__(
        ...
        assert (self.faces.numpy() == flame_model.f.astype('int64')).all()


        #assert (self.faces == torch.tensor(flame_model.f)).all()
...
class FlameMask(nn.Module):
    ...
    def get_headgap_part(self,part_list):
        num_faces=self.faces.shape[0]
        face_part_index=torch.ones(num_faces)*-1
        tensor_list=[]
        for item in part_list:
            matching_attrs = [name for name in dir(self.f) if item in name]
           
            if matching_attrs:
                temp_tensor=self.get_fid_by_region(matching_attrs)
                
                if len(tensor_list) == 0:
                    result = temp_tensor
                else:
                    # 使用 torch.isin 查找 B 中哪些元素在 A 中
                    mask = torch.isin(temp_tensor, torch.cat(tensor_list))
                    # 通过反转 mask，保留 B 中不在 A 中的元素
                    result = temp_tensor[~mask]
                tensor_list.append(result)

        for i in range(0,len(tensor_list)):
            face_part_index[tensor_list[i]]=i
        face_part_index[face_part_index == -1] = 10
        face_part_index = face_part_index.to(torch.int64)
        return  face_part_index       
